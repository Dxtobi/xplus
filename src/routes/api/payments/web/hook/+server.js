	...
	try {
		...
		switch (event.event) {
			...
			case 'transfer.success':
				...
				// The user's balance is already deducted, so no action is needed here.
				break;

			case 'transfer.failed':
			case 'transfer.reversed':
				transaction.status = 'failed';
				transaction.description = event.data.failure_reason || 'Transfer failed or was reversed.';
				await transaction.save();

				// CRITICAL: Refund the amount to the user's wallet
				await User.findByIdAndUpdate(transaction.userId, {
					$inc: { balance: transaction.amount }
				});
				
				// Optional: Send a notification to the user about the failure.
				break;
