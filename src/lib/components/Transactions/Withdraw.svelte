...
<div class="bg-neutral-800 p-6 rounded-2xl border border-neutral-700 w-full max-w-md mx-auto" in:fly={{delay:50, duration:500, x:200, y:0}} out:fly={{delay:50, duration:500, x:-200, y:0}}>
	<div class="flex justify-between">
        <h2 class="text-xl font-bold text-white mb-4">Withdraw Earnings</h2>
        <!-- svelte-ignore a11y_consider_explicit_label -->
        <button onclick={closeModel} class=""><iconify-icon icon="lets-icons:close-ring-duotone" width="24" height="24"  style="color: #fff"></iconify-icon></button>
    </div>
		...
		<div class="space-y-5">
			<div class="bg-neutral-900/50 p-4 rounded-xl border border-neutral-700">
				...
				<div class="flex items-start gap-3 mt-1">
					...
					<iconify-icon icon="solar:banknote-2-bold-duotone" class="text-2xl text-green-400" />
					<div>
						<p class="font-semibold text-neutral-100">{user.bankName}</p>
						<p class="text-xs text-neutral-400">**** **** **** {user.accountNumberLast4}</p>
					</div>
				</div>
			</div>
			...
			<form onsubmit={requestWithdrawal} class="space-y-4">
				<div>
					<label for="withdrawalAmount" class="block text-sm font-medium text-neutral-300 mb-2">
	...
	{:else}
		<form onsubmit={addBankAccount} class="space-y-4">
			<p class="text-sm text-neutral-400">Add a bank account to receive your earnings.</p>
			<div>
				<label for="bank" class="block text-sm font-medium text-neutral-300 mb-2">Select Bank</label>
				<select
					id="bank"
			...
			<div>
				<label for="accountNumber" class="block text-sm font-medium text-neutral-300 mb-2">Account Number</label>
				<input
					type="text"
					id="accountNumber"
					value={accountNumber}
					...
					maxlength="10"
					inputmode="numeric"
					class="w-full p-3 bg-neutral-700/50 border border-neutral-600 rounded-xl text-white focus:ring-2 focus:ring-green-500 {!isValid ? 'border-red-500' : ''}"
					placeholder="Enter 10-digit account number"
				/>
				{#if !isValid && accountNumber.length > 0}
					<p class="text-red-500 text-sm mt-1">Please enter exactly 10 digits</p>
				{/if}
			</div>

			<button
				type="submit"
				disabled={isAddingAccount || !selectedBankCode || accountNumber.length !== 10}
				class="w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold py-3 rounded-xl transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
			>
				{#if isAddingAccount}
					<iconify-icon icon="svg-spinners:180-ring-with-bg" />
