...
<div class="relative inline-block">
	...
	{#if isOpen}
		...
		>
			<div
				class="bg-neutral-800 rounded-t-2xl border-t-2 border-green-500 shadow-2xl overflow-hidden"
			>
...
{#snippet modalContent()}
	...
	<div class="bg-neutral-900/50 px-6 py-4 border-b border-neutral-700">
		<div class="flex items-center justify-between">
			<h3 class="text-lg font-semibold text-neutral-100 flex items-center gap-3">
				<iconify-icon icon="solar:wallet-money-bold-duotone" class="text-green-400" />
				Top Up Wallet
			</h3>
			<button
				onclick={closeModal}
		...
		{#if baseAmount > 0}
			...
			>
				<div class="flex justify-between items-center text-sm">
					...
					<span class="font-semibold text-neutral-200">₦{baseAmount.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</span>
				</div>

				<div class="flex justify-between items-center text-sm">
					<span class="text-neutral-400">Service Fee (10%):</span>
					<span class="font-semibold text-neutral-200">₦{fee.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</span>
				</div>
				...
				<div class="flex justify-between items-center">
					...
					<span class="font-bold text-lg text-green-400">₦{totalAmount.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</span>
				</div>
			</div>
		{/if}
